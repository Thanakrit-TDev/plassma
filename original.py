...
def get_model_from_internet():
    # get json model list from server
    # with open('modellist\\listmode.json', 'r') as file:
    #     data_load = json.load(file)
    # response = {
    #     "model_list_all": data_load,
    # }
...
def sum_static_history(start_date,end_date,mode):
    ...
    # #  'Day', 'Week', 'Month', 'Year'
            ...
            data_all["false_tube"] = {"no data.":0}
            data_all["true_tube"] = {"no data.":0}
            data_all["souse"] = {"no data.":0}
            print(sum_his_lost)
            print(sum_his_true)
            print(souse_data)
            return()
...
def get_log():
    ...
    try:
        ...
        with open(file_path, 'r') as file:
            data_load = json.load(file)
        return jsonify(data_load[0:5]), 200
...
def core1():
    ...
    global brightness, contrast, saturation_boost, range_detect, cal_px, calh
            ...
            while True:
                ...
                if a != -1 and b != -1:
                    ...
                    if img is not None:
                        ...
                        # cv2.rectangle(img, (250,50), (400,450), (255, 0, 0), 5)
                        ...
                        for contour in contours:
                            if cv2.contourArea(contour) > range_detect:
                                ...
                                if x >= rect_x1 and y >= rect_y1 and (x + w) <= rect_x2 and (y + h) <= rect_y2:
                                    ...
                                    img = cv2.putText(img, f'PW:{cal_px} Px', (calw + 10, po_w_box), cv2.FONT_HERSHEY_SIMPLEX,
                                                    0.5, (255, 255, 0), 1, cv2.LINE_AA)
                        cv2.rectangle(img, (rect_x1, rect_y1), (rect_x2, rect_y2), (255, 0, 0), 5)
...
if __name__ == '__main__':
    ...
    c5 = threading.Thread(target=run_read_scan_qrcode)
    c5.daemon = True
    c5.start()
