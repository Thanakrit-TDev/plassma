...
# def core1():
#     global camera
#     global img, running, mask, h_min, h_max, s_min, s_max, v_min, v_max
#     global brightness, contrast, saturation_boost, range_detect, cal_px, calh,frame
...
#         ret, frame = cap.read()
    ...
    #     bytes_data = bytes()
    #     for chunk in stream.iter_content(chunk_size=100000): #1024
    #         if not running:
    #             break
    ...
    #                 if img is not None:
        ...
        # cv2.imshow("window_name", frame)
                ...
                # else:
                #     # Not a complete frame yet, need to wait for more data
                #     break
    # else:
        # print("Failed to access the video feed. Status code:", stream.status_code)
...
# def index():
    ...
    # global img, running, mask, h_min, h_max, s_min, s_max, v_min, v_max
    # global brightness, contrast, saturation_boost, range_detect, cal_px, calh,frame
    # # global forshow_frame
    # # if forshow_frame is None:
...
def Ai_pr():
    ...
    #     print("Error: Could not open video stream.")
    # else:
    #     ret, frame = cap.read()# listcamera_use
    ...
    #     response = requests.get('http://127.0.0.1:3501/video_feed_process_off')
    #     while True:
    #         if(response.status_code == 200):
    #             break
        ...
        if(global_ser_scaner.is_open):
            ...
            while True:
                if(dataQr_fromscaner != ""):
                    data_return_to_app = {"qrdata":dataQr_fromscaner,"hight":cal_px}
                    status_pesent_detect = {"qrdata":dataQr_fromscaner,"hight":cal_px}
                    break
            # Ai zone -------------------------
            Ai_pr()
            # end -----------------------------
            # time.sleep(1)
    return jsonify({"st": "ok","qrdata":dataQr_fromscaner,"hight":cal_px}), 200
...
# @app_4.route("/get_setting_mask", methods=["GET"])
# def test_setting_get():
#     file_path = os.path.join(os.path.dirname(__file__), 'set_detect_yellow.json')
#     with open(file_path, 'r') as file:
#         data_load = json.load(file)
#     return jsonify(data_load), 200

# @app_4.route("/save_setting_detect_yellow")
# def save_setting_detect_yellow():
#     file_path = os.path.join(os.path.dirname(__file__), 'set_detect_yellow.json')
#     with open(file_path, 'w') as file:
#         data = {
#             "h_min":h_min,
...
#         ret, buffer = cv2.imencode('.jpg', frame)
#         frame = buffer.tobytes()
#         # Yield frame and mask
#         yield frame, mask
    
#     cv2.destroyAllWindows()


# @app_test_vdo.route('/video_feed_process')
# def video_feed_process():
#     return Response(generate_frames_process(),
#                     mimetype='multipart/x-mixed-replace; boundary=frame')


# @app_test_vdo.route('/video_feed_process_mask')
# def video_feed_process_mask():
#     return Response(generate_frames_process_mask(),
#                     mimetype='multipart/x-mixed-replace; boundary=frame')
# def get_frame():
#     for frame, _ in generate_frames_process():
#         yield (b'--frame\r\n'
#                b'Content-Type: image/jpeg\r\n\r\n' + frame + b'\r\n')

# def get_mask():
#     for _, mask in generate_frames_process():
...
# @app_test_vdo.route('/video_feed_process')
# def get_fr():
#     return Response(stream_with_context(get_frame()), mimetype='multipart/x-mixed-replace; boundary=frame')
# @app_test_vdo.route('/video_feed_process_mask')
# def get_mask_route():
#     return Response(stream_with_context(get_mask()), mimetype='multipart/x-mixed-replace; boundary=frame')

# def get_frame():
#     global camera
#     camera.set(cv2.CAP_PROP_FRAME_WIDTH, 640)
#     camera.set(cv2.CAP_PROP_FRAME_HEIGHT, 480)
#     camera.set(cv2.CAP_PROP_FPS, 15)  # Adjust the FPS as needed
#     while True:
#         success, frame = camera.read()
#         if not success:
#             break
...
#             cv2.imshow("h",frame)
#             cv2.waitKey(1)
#             frame = cv2.resize(frame, (640, 480))  # Adjust to your preferred size
...
def video_feed_process_on():
    ...
    st_show_image = True
    return jsonify({"ok":"ok"}),200
# @app_test_vdo.route('/video_feed_process_off')
# def video_feed_process_off():
#     global st_show_image,camera
#     st_show_image = False
#     cv2.destroyAllWindows()
#     # camera.release()
#     return jsonify({"ok":"ok"}),200
